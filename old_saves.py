...
def astronaut_selection():
    ...
    if request.method == 'GET':
        ...
        data = {'surname': request.form['surname'],
                'name': request.form['name'], 'education': request.form['obr']}
...
data_default = {'surname': 'Watny', 'name': 'Mark',
                'education': 'выше среднего', 'profession': 'штурман марсохода',
                'sex': 'male', 'motivation': 'Всегда хотел застрять на Марсе!',
                'ready': True}
...
def answer(data=data_default.copy()):
    if data == data_default:
        return auto_answer()
    field_names = ['Фамилия:', 'Имя:', 'Образование:', 'Профессия:', 'Пол:',
                   'Мотивация:', 'Готовы остаться на Марсе?']
    url_style = url_for('static', filename='css/style_ans.css')
    params = {'title': 'Анкета', 'data': data, 'field_names': field_names,
              'url_style': url_style}
...
def login():
    ...
    params = {'title': 'Аварийный доступ', 'form': form,
              'url_emblem': url_emblem, 'url_style': url_style_login}
...
def table(pol, age):
    ...
    params = {'url_style': url_style, 'url_emblem': url_emblem,
              'url_color': url_color}
